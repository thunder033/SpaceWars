...
void GameObject::prepareMaterial(DirectX::XMFLOAT4X4 viewMatrix, DirectX::XMFLOAT4X4 projectionMatrix, ID3D11SamplerState* sampler)
{
	...
	frameMaterial->getVertexShader()->SetMatrix4x4("world", getDrawMatrix());
	frameMaterial->getVertexShader()->SetMatrix4x4("view", viewMatrix);
	frameMaterial->getVertexShader()->SetMatrix4x4("projection", projectionMatrix);

	frameMaterial->getPixelShader()->SetData("Color", &frameMaterial->getColor(), sizeof(DirectX::XMFLOAT4));

	frameMaterial->getPixelShader()->SetSamplerState("Sampler", sampler);
	frameMaterial->getPixelShader()->SetShaderResourceView("Texture", frameMaterial->getTexture());

	// Once you've set all of the data you care to change for
	// the next draw call, you need to actually send it to the GPU
	//  - If you skip this, the "SetMatrix" calls above won't make it to the GPU!
	frameMaterial->getVertexShader()->CopyAllBufferData();
	frameMaterial->getPixelShader()->CopyAllBufferData();

	// Set the vertex and pixel shaders to use for the next Draw() command
	//  - These don't technically need to be set every frame...YET
	//  - Once you start applying different shaders to different objects,
	//    you'll need to swap the current shaders before each draw
	frameMaterial->getVertexShader()->SetShader();
	frameMaterial->getPixelShader()->SetShader();
